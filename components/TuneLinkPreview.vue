...
function buildIframelyURL(link: string) {
  return `https://cdn.iframe.ly/api/iframely?url=${encodeURI(
    link
...
<template>
  <button class="tune-link-preview-wrapper flex w-full items-center">
    <div class="shrink-0 px-4">
      ...
      <div
        v-else
        class="tune-link-preview-icon rounded-lg"
      >
        <IconTuneDiscord
    ...
    <div class="overflow-hidden py-4 pr-4">
      <div
        v-if="!preview?.meta.title || loading"
        class="flex h-[40px] flex-col justify-center"
      >
