...
const imageUrl = computed(() => {
  const cacheParam = props.cacheBreaker
    ? `&cb=${getCacheHash(props.cacheBreaker)}`
    : ''
...
const spaceImgRadius = computed(() => {
  return typeof props.size === 'number' && props.size >= 48
    ? 'space-style-lg'
    : 'space-style'
})
